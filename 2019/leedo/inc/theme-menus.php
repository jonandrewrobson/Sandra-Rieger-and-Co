 ...
 * Custom nav walker
 */
if ( ! function_exists( 'Custom_Walker_Nav_Menu' ) ) {
	class Custom_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $depth == 0 && class_exists( 'acf' ) && get_field( 'megamenu', $item ) ) {
				$classes[] = 'menu-item-has-megamenu';

				$columns = get_field( 'columns', $item );
				if ( $columns ) {
					$classes[] = 'menu-item-has-megamenu--' . $columns . 'col';
				}

				if ( get_field( 'show_label', $item ) ) {
					$classes[] = 'menu-item-megamenu-show-label';
				}
			}
			/**
