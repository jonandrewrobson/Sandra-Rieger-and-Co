 ...
 * ACF in Admin Panel
 */
if ( ! function_exists( 'leedo_acf_in_admin_panel' ) ) {
	function leedo_acf_in_admin_panel() {
		return leedo_get_theme_mod( 'acf_show_admin_panel' ) == 'show' ? true : false;
	}
}
add_filter( 'leedo/acf_show_admin_panel', 'leedo_acf_in_admin_panel' );
...
if ( ! function_exists( 'leedo_add_html_class' ) ) {
	function leedo_add_html_class( $classes = '' ) {
		...
		if ( leedo_get_theme_mod( 'mobile_navigation_sticky' ) == 'enable' ) {
			$classes .= ' vlt-is--mobile-menu-fixed';
		}
		...
		$classes .= ( leedo_get_theme_mod( 'site_protection', $acf_page_custom_site_protection ) == 'show' && ! current_user_can( 'administrator' ) ) ? ' vlt-is--site-protection' : '';
