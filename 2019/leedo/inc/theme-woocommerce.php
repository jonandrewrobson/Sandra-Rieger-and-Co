...
/**
 * Add ordering to WooCommerce
 */
if ( ! function_exists( 'leedo_woocommerce_get_catalog_ordering_args' ) ) {
	function leedo_woocommerce_get_catalog_ordering_args( $args ) {
...
add_filter( 'woocommerce_get_catalog_ordering_args', 'leedo_woocommerce_get_catalog_ordering_args' );

if ( ! function_exists( 'leedo_woocommerce_catalog_orderby' ) ) {
	function leedo_woocommerce_catalog_orderby( $sortby ) {
		$sortby['oldest_to_newest'] = esc_html__( 'Sort oldest to newest', 'leedo' );
		return $sortby;
	}
}
add_filter( 'woocommerce_default_catalog_orderby_options', 'leedo_woocommerce_catalog_orderby' );
add_filter( 'woocommerce_catalog_orderby', 'leedo_woocommerce_catalog_orderby' );

/**
 * Minicart subtotal
 */
if ( ! function_exists( 'leedo_shopping_cart_total' ) ) {
	function leedo_shopping_cart_total() {
		echo '<strong class="vlt-display-1">' . esc_html__( 'Subtotal', 'leedo' ) . ':</strong> ' . WC()->cart->get_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
add_action( 'woocommerce_widget_shopping_cart_total', 'leedo_shopping_cart_total' );

/**
 * Fix lazyload Jetpack
 */
if ( ! function_exists( 'leedo_exclude_custom_class_lazy_load' ) ) {
	function leedo_exclude_custom_class_lazy_load( $classes ) {
		$classes[] = 'attachment-woocommerce_thumbnail';
		return $classes;
	}
}
add_filter( 'jetpack_lazy_images_blacklisted_classes', 'leedo_exclude_custom_class_lazy_load', 999, 1 );