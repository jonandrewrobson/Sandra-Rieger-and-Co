...
if ( ! class_exists( 'ThemeEnqueueAssets' ) ) {
	class ThemeEnqueueAssets {
		...
		public function enqueue_default_scripts( $wp_scripts ) {
			...
			if ( leedo_get_theme_mod( 'jquery_in_footer' ) == 'disable' ) {
				return;
			}
		...
		public function enqueue_frontend_scripts() {
			...
			if ( leedo_get_theme_mod( 'nprogress' ) == 'enable' ) {
			...
			wp_localize_script(
				...
				'VLT_SUBSCRIBE_CDATA',
				array(
					'subscribe_thank_you_page' => get_permalink( leedo_get_theme_mod( 'mailchimp_thank_you_page' ) ),
			...
			if ( leedo_get_theme_mod( 'menu_toggle_sound' ) == 'enable' ) {
				wp_localize_script(
					'vlt-controllers',
		...
		public function enqueue_frontend_styles() {
			...
			wp_enqueue_style( 'bootstrap', $this->assets_dir .'css/framework/bootstrap.min.css', array(), $this->theme_version );
			...
			wp_enqueue_style( 'leedo', $this->assets_dir .'fonts/leedo/style.css', array(), $this->theme_version );
			wp_enqueue_style( 'font-awesome-5', '//use.fontawesome.com/releases/v5.8.1/css/all.css', array(), $this->theme_version );
			wp_enqueue_style( 'vlt-main-css', $this->assets_dir .'css/vlt-main.min.css', array(), $this->theme_version );
			if ( ! class_exists( 'Kirki' ) ) {
